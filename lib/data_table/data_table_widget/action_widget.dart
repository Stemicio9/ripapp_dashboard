...
class ActionWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if(action.isButton){
      return SuperiorActionWidget(action: action.action, text: action.text);
    } else {
      return IconTableWidget(icon: action.icon, tooltip: action.tooltip);
    }
  }
}
...
class IconTableWidget extends StatelessWidget {
  ...
  const IconTableWidget({Key? key, required this.icon, required this.tooltip}) : super(key: key);
...
class SuperiorActionWidget extends StatelessWidget {
  ...
  const SuperiorActionWidget({Key? key, required this.text, required this.action}) : super(key: key);
